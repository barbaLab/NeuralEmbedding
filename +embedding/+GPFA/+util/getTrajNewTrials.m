...
% Aug 21, 2011: Added option for causal smoothing
  ...
  if ismember(ws.method, {'gpfa'}) & ~causal
    seqNew         = embedding.GPFA.core_gpfa.exactInferenceWithLL(seqNew, ws.estParams);
    C              = ws.estParams.C;
    X              = [seqNew.xsm];
    [Xorth, Corth] = embedding.GPFA.util.orthogonalize(X, C);
    seqNew         = segmentByTrial(seqNew, Xorth, 'xorth');

  elseif ismember(ws.method, {'gpfa'}) & causal
    seqNew         = causalInference(seqNew, ws.estParams, extraOpts{:});
    C              = ws.estParams.C;
    X              = [seqNew.xfi];
    [Xorth, Corth] = embedding.GPFA.util.orthogonalize(X, C);
    seqNew         = segmentByTrial(seqNew, Xorth, 'xorth');
  ...
  elseif ismember(ws.method, {'fa', 'ppca'})
    ...
    [Xorth, Lorth] = embedding.GPFA.util.orthogonalize(X.mean, L);
    seqNew         = segmentByTrial(seqNew, Xorth, 'xorth');
