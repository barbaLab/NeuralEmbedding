...
% @ 2009 Byron Yu -- byronyu@stanford.edu
  ...
  if rank(cX) == xDim
    ...
    % check if it is positive semidefinite
    r     = rank(cX);
  ...
  for i = 1:cyc
    ...
    LLold = LLi;    
    [chol_MM flag] = chol(MM);
    if (flag ~= 0) % the matrix is not positive semi-definite
        fprintf(['ERROR: Matrix is not positive semi-definite.\n' ...
                 'Try removing neurons with low mean firing rate\n (<1.0 spikes/sec).\n']);
        estParams = [];
        LL = NaN;
        return;
    end
    ldM   = sum(log(diag(chol_MM)));
