...
%        John Cunningham  jcunnin@stanford.edu
  ...
  verbose      = false;
  freqLL       = 10;
  extra_opts   = embedding.GPFA.util.assignopts(who, varargin);
  ...
  for i = 1:emMaxIters
    ...
    [seq, LLi] = embedding.GPFA.core_gpfa.exactInferenceWithLL(seq, currentParams, 'getLL', getLL);    
    LL         = [LL LLi];
    ...
    if currentParams.notes.learnKernelParams
      res = embedding.GPFA.core_gpfa.learnGPparams(seq, currentParams, 'verbose', verbose,... 
                          extra_opts{:});
