...
%        John Cunningham  jcunnin@stanford.edu
  ...
  [blah, Corth, TT] = embedding.GPFA.util.orthogonalize(zeros(xDim, 1), params.C);
  ...
  for j = 1:length(Tu)
    ...
    [K_big, K_big_inv, logdet_K_big] = embedding.GPFA.core_gpfa.make_K_big(params, T);
    
    K_big = sparse(K_big);
    
    blah      = cell(1, T);
    [blah{:}] = deal(CRinvC);    
    invM      = embedding.GPFA.util.invPerSymm(K_big_inv + blkdiag(blah{:}), xDim,...
                           'offDiagSparse', true);
    ...
    for i = 1:yDim      
      ...
      ci_invM = embedding.GPFA.util.fillPerSymm(ci_invM, xDim, T, 'blkSizeVert', 1);    % T x (xDim*T)
      term    = (embedding.GPFA.util.fillPerSymm(ci_invM_ci, 1, T) - eye(T)) \ ci_invM; % T x (xDim*T)
      ...
      blkProd = K_big(1:(xDim*Thalf), :) *...
                  embedding.GPFA.util.fillPerSymm(speye(xDim*Thalf, xDim*T) - blkProd, xDim, T);
      xsmMat  = embedding.GPFA.util.fillPerSymm(blkProd, xDim, T) * term1Mat; % (xDim*T) x length(nList)
