%% New tanglind metric
function [theta ] = alignment(data,type,nsec)
if isempty(data)
    theta = nan;
    return;
end
...
switch type
    case 'equidist'
        Np = nsec;
        t = linspace(.05,.95,Np+2);

        [pt,~,func] = interparc(t,m_cell{:});
        pt = pt';
        ...
        allIdx = nan(nTraj,Np);
        for tt = 1:nTraj % trials
            for ss = 2 : Np+1  % points
                [~,idx] = min(vecnorm(data(:,:,tt) - pt(:,ss)));
                if idx ==1
                    idx = 2;
                end
                allIdx(tt,ss) = idx;
