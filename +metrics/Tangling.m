...
for area = 1:3
    for aa = 1:12
        ...
        for c = 1:numel(mixedConds)
            ...
            theta(aa,c,area) = newTangling(dat,'index',200);
            an{aa,c,area} = aa;
            conds{aa,c,area} = cc;
...
m = mean(data,3);
m_ = arrayfun(@(idx)m(idx,:),1:size(m,1),'UniformOutput',false);


switch type
    case 'equidist'
        t = linspace(0.05,.95,nsec+2);

        [pt,~,func]=interparc(t,m_{:});
        pt = pt';
        % method based on the approximated curve
        pt_1 = func(t+eps)';
        n = pt_1 - pt;
        n = n./vecnorm(n);

        % Main loop: foreach trajectory find the point immediatly before pt and
        % compute the versor m connecting that point to the one after it.
        theta = nan(size(data,3),length(t)-2);
        for tt = 1:size(data,3)
            for ss = 2:length(t)-1
                [val,idx] = min(vecnorm(data(:,:,tt) - pt(:,ss)));
                if idx ==1
                    p = data(:,idx+1,tt) - data(:,idx,tt);
                else
                    p = data(:,idx,tt) - data(:,idx-1,tt);
                end
                p = p./vecnorm(p);
                theta(tt,ss-1) = acos(dot(n(:,ss),p));
            end
        end
    case 'index'
        pt = m(:,nsec);
        ...
        theta = nan(size(data,3),length(nsec));
        for tt = 1:size(data,3)
            for ss = 1:length(nsec)
                p = (data(:,nsec,tt) - data(:,nsec+1,tt));
                p = p./vecnorm(p);

                theta(tt,ss) = acos(dot(n(:,ss),p(:,ss)));
            end
        end
end

theta = mean(std(theta,[],1));
